...
// ✅ Keep speed consistent on all devices
let baseSpeed = 0.6;
...
function renderLines(){
    for(let i=0;i<balls.length;i++){
        for(let j=i+1;j<balls.length;j++){
            const dis = getDisOf(balls[i],balls[j]);
            if(dis < dis_limit){
                const alpha = 1 - dis / dis_limit;
                // 🔹 smoother mesh (lower opacity, subtle glow)
...
function renderShootingStars(){
    for(let i=shootingStars.length-1;i>=0;i--){
        ...
        s.x += s.vx * lineSpeed * 1.5;
        s.y += s.vy * lineSpeed * 1.5;
        s.alpha -= 0.02;
...
window.addEventListener('resize',()=>{
    ...
    lineSpeed = baseSpeed * (window.innerWidth < 768 ? 0.4 : 1);
});
